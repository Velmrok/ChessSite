...
export default function FriendList(){
    ...
    if(friends.length === 0){
        return (
            <>
            <div className="font-MyFancyFont text-lg text-white flex justify-center">{t.profile.friends}</div>
    ...
    return (
        ...
        <div className="font-MyFancyFont text-2xl text-white flex justify-center ">{t.profile.friends}</div>
                 ...
                 bg-gray-900/50 p-2 rounded-md w-full justify-between px-3 sm:px-6 relative">
                    <Link to={`/users/${friend.nickname}/profile`} className="text-xs md:text-sm text-white hover:text-amber-200
                             transition-transform duration-300 rounded-full outline-2 border-2 border-black">
                                <img src={`${API_URL}${friend.avatar}`} alt="Profile" className="h-8 w-8 min-h-8 min-w-8 md:w-12 md:h-12 md:min-h-12 md:min-w-12 rounded-full" />
                            </Link>
