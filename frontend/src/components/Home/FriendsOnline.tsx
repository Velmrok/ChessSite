...
export default function FriendsOnline(){
    ...
    if(friends.length === 0){
        return (
         ...
         w-full mt-4 bg-gray-800/[30%] rounded-lg gap-4 text-white min-h-[300px] relative">
            <div className="font-MyFancyFont text-lg text-white flex justify-center absolute top-3">{t.profile.friends}</div>
    ...
    return (
        ...
        w-full max-w-[540px] mt-4 bg-gray-800/[30%] rounded-lg gap-4">
            <div className="font-MyFancyFont col-span-3 text-2xl text-white flex justify-center ">{t.profile.friends}</div>
            {loadingFriends ? <Loading /> :
                ...
                <img src={`${API_URL}${friend.avatar}`} alt="Profile" className="h-13 w-13 min-h-13 min-w-13 md:w-18 md:h-18
